...
class InferenceModel:
    ...
    def patch_model(self, function_name, attention_parameters={}, ffn_parameters={}, patch_attention=True, patch_ffn=True):
        ...
        print(torch.cuda.get_device_properties(self.device).total_memory)
        ...
        if 'llama' in self.model_name:
            for i, layer in enumerate(self.model.model.layers):
                ...
                ffn_object = ffn_class(**ffn_parameters, layer=i, device=self.device, profile_path=path, profile_dims=self.profiling_dims)
            ...
            for i, layer in enumerate(self.model.model.encoder.layers):
                attention_object = attention_class(**attention_parameters, layer=i, device=self.device, profile_path=path, profile_dims=self.source_profiling_dims)
                ffn_object = ffn_class(**ffn_parameters, layer=i, device=self.device, profile_path=path, profile_dims=self.source_profiling_dims)
            ...
            for i, layer in enumerate(self.model.model.decoder.layers):
                attention_object = attention_class(**attention_parameters, layer=i, device=self.device, profile_path=path, profile_dims=self.target_profiling_dims)
                ffn_object = ffn_class(**ffn_parameters, layer=i, device=self.device, profile_path=path, profile_dims=self.target_profiling_dims)
                eager_attn_fn = WhisperEager(nonlinear_object=attention_object)
                forward = whisper_forward(eager_attn_fn)
            ...
            for i, block in enumerate(self.model.swinv2.encoder.layers):
                for j, layer in enumerate(block.blocks):
                    attention_object = attention_class(**attention_parameters, layer=j, blocks=i, device=self.device, profile_path=path, profile_dims=self.profile_dims)
                    ffn_object = ffn_class(**ffn_parameters, layer=j, blocks=i, device=self.device, profile_path=path, profile_dims=self.profile_dims)
            ...
            for i, layer in enumerate(self.model.vivit.encoder.layer):
                attention_object = attention_class(**attention_parameters, layer=i, device=self.device, profile_path=path, profile_dims=self.profile_dims)
                ffn_object = ffn_class(**ffn_parameters, layer=i, device=self.device, profile_path=path, profile_dims=self.profile_dims)
