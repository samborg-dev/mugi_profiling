...
class VLPSoftmax(CustomSoftmax):
    ...
    def window_softmax_approx(self, exp, mant):
        ...
        if exp.shape[0] % self.window_size != 0:
            ...
            if self.lut_build == "max":
                padded_exp = torch.nn.functional.pad(exp, pad=tuple(padding_shape), value=-1000)
                padded_mant = torch.nn.functional.pad(mant, pad=tuple(padding_shape), value=-1000)
            else:
                padded_exp = torch.nn.functional.pad(exp, pad=tuple(padding_shape), value=1000)
                padded_mant = torch.nn.functional.pad(mant, pad=tuple(padding_shape), value=1000)
        else:
        ...
        padded_exp = padded_exp.view(self.window_size, padded_exp.shape[0] // self.window_size, padded_exp.shape[1])
        padded_mant = padded_mant.view(self.window_size, padded_mant.shape[0] // self.window_size, padded_mant.shape[1])

        # calculate min and max windows
        if self.lut_build == "max":
            max_exp_window = torch.max(padded_exp, dim=0, keepdim=True)[0].expand_as(padded_exp)
            ...
            min_exp_window = torch.min(padded_exp, dim=0, keepdim=True)[0].expand_as(padded_exp)
            min_exp_window = torch.where(min_exp_window < self.min_exp, self.min_exp, min_exp_window)
        ...
        if self.lut_build == "max":
            ...
            mant_window_max = torch.where(padded_exp <= max_exp_window, padded_mant, self.mant_dim - 1)
            mant_window = torch.where(exp_window_max >= min_exp_window, mant_window_max, torch.where(exp_window_max == -1000, mant_window_max, 0))
        else:
            exp_window_min = torch.where(padded_exp >= min_exp_window, padded_exp, min_exp_window)
            ...
            mant_window_min = torch.where(padded_exp >= min_exp_window, padded_mant, 0)
        ...
        # adjust to index lut
        if self.lut_build == "max":
            exp_window = exp_window - self.min_exp
        else:
            exp_window = exp_window - self.max_exp

        # reshape to original shape
        exp_window = exp_window.view(-1, attn_shape[-1])
        mant_window = mant_window.view(-1, attn_shape[-1])

        exp_unpadded = exp_window[:exp.shape[0], :]
        mant_unpadded = mant_window[:mant.shape[0], :]

        exp_out = exp_unpadded.view(*attn_shape)
        mant_out = mant_unpadded.view(*attn_shape)
