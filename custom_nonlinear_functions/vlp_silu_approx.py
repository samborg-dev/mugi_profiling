...
class VLPSilu(CustomSilu):
    def __init__(self, exp_dim, max_pos_exp, max_neg_exp, window_size, device, path, save_dims, profile, torch_nonlinear):
        super(VLPSilu, self).__init__(device, path, save_dims, profile, torch_nonlinear)
    ...
    def nonlinear(self, x):
        ...
        pos_exp, pos_mant, neg_exp, neg_mant = self.window_silu_approx(exp_preprocess, mant_preprocess)

        pos_mant = pos_mant.to(torch.int32)
        neg_mant = neg_mant.to(torch.int32)

        silu = self.lut_index(x, pos_exp, pos_mant, neg_exp, neg_mant, exp_preprocess)
