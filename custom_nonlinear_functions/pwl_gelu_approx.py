...
class PWLGelu(CustomGelu):
    def __init__(self, segments, segment_0, device, path, save_dims, profile, torch_nonlinear):
        super(PWLGelu, self).__init__(device, path, save_dims, profile, torch_nonlinear)
    ...
    def build_lut(self):
        ...
        self.y_segments = self.torch_nonlinear(self.x_segments).to(self.device).to(torch.bfloat16)
